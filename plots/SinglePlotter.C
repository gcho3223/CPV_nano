...
void SinglePlotter(string version, string period, string channel)
{
	...
	for(int isn=0; isn<sample.size(); isn++)
	{
		TString fin;
		/// file read ///
		fin = Form("../output/%s/%s/%s/%s.root",version.c_str(),period.c_str(),channel.c_str(),sample[isn].c_str());
		...
		for(int ih1=0; ih1<hist1D.size(); ih1++)
		{
			...
			DrawHist1D(hist1d,sample[isn],version,savepath);
		}
